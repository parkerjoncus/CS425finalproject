...
def main():
    ...
    while (command2==1 or command2==2 or command2==3 or command2==4 or command2==5 or command2==6):
        ...
        if command2==1:
            ...
            cur.execute('SELECT * FROM schedule s, ticket t WHERE s.date=t.date AND s.departuretime=t.departuretime AND iata_from=%s AND iata_to=%s',(from_airport,to_airport))
            rows = cur.fetchall()
            if rows == []:
